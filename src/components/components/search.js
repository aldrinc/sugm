...
class Search extends Component {
  ...
  componentWillMount() {
    ...
    if (product) this.setState({ products: product });
  }
  ...
  handleInputChange = () => {
    ...
    if (
      ...
      const searchProduct = this.state.products.filter(product =>
        product.title.toLowerCase().includes(this.search.value.toLowerCase())
      );
          ...
          return (
            <div className="searchProduct">
              <div className="row">
                ...
                <div className="col-8">
                  <Link
                    to={`/product/${pro.handle}`}
                    onClick={this.props.hideSearch}
                  >
                    {pro.title.substring(0, 50)}{" "}
                    {pro.title.length > 50 ? "..." : ""}
                  </Link>
                </div>
              </div>
